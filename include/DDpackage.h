...
namespace dd {
    ...
    struct Node {
	    NodePtr next;         //下一个结点的指针 link for unique table and available space chain
	    Edge e[NEDGE];     //该结点的出边 edges out of this node
	    Complex normalizationFactor; //规范化因子 stores normalization factor
	    unsigned int ref;       //结点引用技术 reference count
	    short v;        //变量的序号(索引) variable index (nonterminal) value (-1 for terminal)
	    bool ident, symm; //特殊矩阵的标志 special matrices flags
	    ComputeMatrixPropertiesMode computeMatrixProperties; // indicates whether to compute matrix properties
	    unsigned int reuseCount; //重用计数
    };
