...
namespace dd {
    ...
    ComplexTableEntry *ComplexNumbers::lookupVal(const fp &val) {
        ...
        auto p = ComplexTable[key]; //根据key找到对应的桶指针
        while (p != nullptr) {
        ...
        //如果直接用val的key找不到,那么尝试用上误差值
        if (val - TOLERANCE >= 0) {
            const auto key2 = getKey(val - TOLERANCE);
            if (key2 != key) {
                p = ComplexTable[key2];
                while (p != nullptr) { //在误差值上找
                    if (std::fabs(p->val - val) < TOLERANCE) {
                        return p;
                    }
                    p = p->next;
        ...
        ct_miss++; //更新找不到的计数
        auto *r = getComplexTableEntry(); //那就新建一个
        r->val = val;
    ...
    Complex ComplexNumbers::lookup(const Complex &c) {
        ...
        if (absi < TOLERANCE) { //虚部的值比误差值还要小,也就是说虚部不用不用处理了
            if (absr < TOLERANCE)
                return ZERO;
            ...
            ret.r = lookupVal(absr); //利用实部找
            if (sign_r) //这里是处理符号位
                ret.r = (ComplexTableEntry *) (((uintptr_t) (ret.r)) | 1u);
            return ret;
        }

        if (absr < TOLERANCE) { //实部的值比误差值还要小,实部没有处理的必要,只处理虚部
