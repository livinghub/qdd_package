...
namespace dd {
	...
	void Package::exchangeBaseCase(unsigned short i, Edge in) {
		...
		for (unsigned short bucket=0; bucket < NBUCKET; ++bucket) { //遍历i变量的表的每个桶
			table.at(bucket) = Unique[i][bucket]; //把每个桶的东西都拿出来
			Unique[i][bucket] = nullptr; //把唯一表清空
		}

		initComputeTable(); //初始化计算表

		// iterate over all obtained nodes
		for (unsigned short bucket=0; bucket < NBUCKET; ++bucket) { //遍历每个桶
			NodePtr p = table[bucket];
			while (p != nullptr) { //对每个桶里面的链的每个i变量的结点进行处理
				...
				if (p->ref != 0) {
                    exchangeBaseCase2(p, i, in); //传入结点,变量序号,dd指针
				}
                assert(p->v == i);
                assert(i-1 == p->e[0].p->v || isTerminal(p->e[0]));
                assert(i-1 == p->e[1].p->v || isTerminal(p->e[1]));
...
/// \param varMap 顺序和逆序的映射 stores the variable mapping (cf. dynamicReorder(...))
/// \return the resulting decision diagram (and the changed variable map and output permutation, which are returned as reference)
    std::tuple<Edge, unsigned int, unsigned int> Package::sifting(Edge in, std::map<unsigned short, unsigned short>& varMap) {
		const auto n = static_cast<short>(in.p->v + 1); //dd的结点个数
...
//		}
		...
		unsigned int total_max = size(in); //要处理的最大结点数
        ...
        for (int i = 0; i < n; ++i) { //遍历每个结点
            ...
            for (short j = 0; j < n; j++) {
                if (free.at(varMap[j]) && active.at(varMap[j]) > max) {
                    max = active.at(varMap[j]); //记录变量的最大结点数
                    pos = j; //最大结点数的那个位置
            ...
            if (pos < n / 2) {  // variable is in lower half -> sifting to bottom first
                // sifting to bottom
                while (pos > 0) {
                    exchangeBaseCase(pos, in); //传入位置和dd指针
